...
data_dir = r'E:\ImageProcess\Cifar100'
...
def _load_batch_cifar100(filename, dtype='float64'):
    ...
    labels = one_hot(batch['fine_labels'], n=100)  # "fine"小类标签,"coarse"大类标签
    return data.astype(dtype), labels.astype(dtype)


def cifar100(dtype='float64', grayscale=False):
    ...
    # save the hdf5 files
    repo_path = os.path.expandvars(os.path.expanduser(data_dir))
